...
def generate_qr_from_cert(cert_file, output_file=None, output_dir="output"):
    ...
    try:
        ...
        print(f"Certificate size: {len(cert_pem.encode('utf-8'))} bytes (PEM)")
...
def main():
    parser = argparse.ArgumentParser(description='Generate QR code from PKI certificate')
    parser.add_argument('cert_file', help='Path to certificate file (.pem)')
    ...
    if not args.output:
        ...
        base_name = cert_basename.replace('.pem', '').replace('-cert', '')
