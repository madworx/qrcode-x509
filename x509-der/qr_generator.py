...
def generate_qr_from_cert(cert_file, output_file=None, output_dir="output"):
    ...
    try:
        ...
        # Read DER certificate file as binary
        with open(cert_file, 'rb') as f:
            cert_der = f.read()

        # Create QR code with raw binary data (no Base64 encoding)
        qr = qrcode.QRCode(
        ...
        print(f"Certificate size: {len(cert_der)} bytes (DER)")
...
def main():
    parser = argparse.ArgumentParser(description='Generate QR code from PKI certificate (DER format)')
    parser.add_argument('cert_file', help='Path to certificate file (.der)')
    ...
    if not args.output:
        ...
        base_name = cert_basename.replace('.der', '').replace('-cert', '')
