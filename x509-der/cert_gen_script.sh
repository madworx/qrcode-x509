...
Certificate output: [serial]-cert.der (DER format)
EOT
    exit 1
fi
...
# Sign the certificate with explicit serial number (output as DER)
openssl x509 -req -in "${OUTPUT_DIR}/${SERIAL_NUM}.csr" \
    -CA "${OUTPUT_DIR}/ca.crt" -CAkey "${OUTPUT_DIR}/ca.key" \
    -out "${OUTPUT_DIR}/${SERIAL_NUM}-cert.der" \
    -outform DER \
    -days "$DAYS" \
...
# Display certificate info (convert DER to text for display)
echo "Certificate generated: ${OUTPUT_DIR}/${SERIAL_NUM}-cert.der"
openssl x509 -in "${OUTPUT_DIR}/${SERIAL_NUM}-cert.der" -inform DER -noout -text | grep -E "(Serial Number|Subject:|Not Before|Not After)"

# Get certificate size
echo "Certificate size: $(wc -c < "${OUTPUT_DIR}/${SERIAL_NUM}-cert.der") bytes (DER)"
